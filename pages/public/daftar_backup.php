...
$query = "SELECT b.*, u.username 
          FROM backup_db b
          JOIN user u ON b.id_admin = u.id_user
...
<body class="layout-fixed sidebar-expand-lg bg-body-tertiary">
    ...
    <div class="app-wrapper">
        ...
        <main class="app-main">
            ...
            <div class="app-content-header my-3 mb-5">
                <!--begin::Container-->
                <div class="container-fluid">
                    <!--begin::Row-->
                    <div class="row">
                        <div class="col-sm-12 text-center">
                            <h3 class="mb-0 fw-bold font-monospace fs-1">Daftar Backup Database</h3>
                        </div>
                    </div>
                    <!--end::Row-->
                </div>
                            ...
                            <div class="card card-success card-outline">
                                <div class="card-header">
                                    ...
                                    <div class="fs-5 float-end">
                                        <button id="downloadAllBtn" class="btn btn-sm btn-primary text-white mb-1">
                                                ...
                                                <?php if (mysqli_num_rows($result) > 0): ?>
                                                    ...
                                                    <?php while ($row = mysqli_fetch_assoc($result)): ?>
                                                        <tr>
                                                            ...
                                                            <td><?= $row['nama_file'] ?></td>
                                                            <td><?= date('d-m-Y H:i:s', strtotime($row['tanggal_backup'])) ?></td>
                                                            <td><?= $row['username'] ?></td>
                                                            <td>
                                                                <a href="../../src/config/download-sql.php?id=<?= $row['id_backup'] ?>" class="btn btn-sm btn-primary text-white mb-1" data-bs-trigger="hover" data-bs-placement="bottom" data-bs-custom-class="custom-tooltip-Download" data-bs-title="Download">
                                                                    <i class="fas fa-download"></i>
                                                                </a>
                                                                <button class="btn btn-sm btn-danger btn-hapus mb-1" data-id="<?= $row['id_backup'] ?>" data-bs-trigger="hover" data-bs-placement="bottom" data-bs-custom-class="custom-tooltip-Delete" data-bs-title="Delete ( Hapus )">
            ...
            document.querySelectorAll('.btn-hapus').forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    Swal.fire({
                        ...
                        if (result.isConfirmed) {
                            // Kirim request ke server untuk menghapus backup
                            fetch('../../src/config/hapus_backup.php?id=' + id)
                                .then(response => response.json())
                                .then(data => {
                                    if (data.success) {
                                        Swal.fire({
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.getElementById('downloadAllBtn').addEventListener('click', function(e) {
                ...
                Swal.fire({
                    ...
                    text: "Apakah Anda yakin ingin mengunduh semua file backup?",
                    icon: 'question',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',
                    ...
                    if (result.isConfirmed) {
                        Swal.fire({
                            title: 'Memproses...',
                            text: 'Sedang menyiapkan file backup',
                            icon: 'info',
                        ...
                        // Request ke server untuk membuat file ZIP
                        fetch('../../src/config/download-all.php')
                            .then(response => response.json())
                            .then(data => {
                                if (data.success) {
                                    // Download file ZIP
                                    window.location.href = '../../src/config/download.php';
